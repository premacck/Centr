...
/**
 * 
 * Created by Prem $ on 8/12/2017.
 *'
 * 
 * TOAST like android CLASS
 *
 * <p>you create it by<br>Toast render_toast = new Toast(7, 6);</p>
 * <p>then you feed it a new text<br>render_toast.makeText("test string", "font", Toast.COLOR_PREF.RED, Toast.STYLE.NORMAL, Toast.POSITION.middle, Toast.POSITION.middle, Toast.ETERNAL);</p>
...
public class Toast {
    ...
    private int max_toasts = 15; // default max_toasts
...
//    public static final int LONG = 8;
    private static final int ETERNAL = -1;
...
//    }
     ...
     *  time time desired into seconds
     *  _font font desired, should lie in /data/"_font".png/fnt
     *  prefered_color color (see {@linkplain COLOR_PREF}) that is prefered (it will random for the 2 other colors, but for this one, it will stay at 1.0f)
     *  style fading, blinking .. style (see {@link POSITION})
     *  x where we want the text on X coords
     *  y where we want the text on Y coords, it will offset by number of total texts to display in this Toast
     *  time_to_spend the time to diplay THIS text
     * */
    public boolean makeText(String string, int prefered_color, int style, float x, float y, float time_to_spend) {
        ...
        font[i] = new BitmapFont(Gdx.files.internal("cranberry_gin_20.fnt"), Gdx.files.internal("cranberry_gin_20.png"), false);

        GlyphLayout bounds = new GlyphLayout();
        bounds.setText(font[i], string);
        font_width[i] = (int) (bounds.width + margin); // width of the string + margin
        font_height[i] = (int) font[i].getLineHeight() + margin; // height of the string
        float x_bounds = bounds.width / 2; // we center the string
        ...
        else if (x == POSITION.middle_left)
            X[i] = (width/3) - x_bounds;
        else if (x == POSITION.middle_right)
            X[i] = (width - (width/3)) -x_bounds;
        if (y == POSITION.middle)
            Y[i] = (height/2) + y_bounds;
        else if (y == POSITION.middle_down)
            Y[i] = (height/3) + y_bounds;
        else if (y == POSITION.middle_up)
